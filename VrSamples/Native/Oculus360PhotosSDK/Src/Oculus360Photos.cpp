...
ovrFrameResult Oculus360Photos::Frame( const ovrFrameInput & vrFrame )
{
	...
	if ( GetUseOverlay() && CurrentPanoIsCubeMap )
	{
		...
		FrameParms.LayerCount = 2;
		{
			...
			{
				layer.Textures[eye].ColorTextureSwapChain = FrameInput.ColorTextureSwapChain[eye];
				layer.Textures[eye].DepthTextureSwapChain = FrameInput.DepthTextureSwapChain[eye];
				layer.Textures[eye].TextureSwapChainIndex = FrameInput.TextureSwapChainIndex;
				layer.Textures[eye].TexCoordsFromTanAngles = FrameInput.TexCoordsFromTanAngles;
		...
		}
		{
			...
			for ( int eye = 0; eye < 2; eye++ )
			{
				...
				layer.Textures[eye].TexCoordsFromTanAngles = CubeMatrixForViewMatrix( res.FrameMatrices.CenterView );
				layer.Textures[eye].HeadPose = FrameInput.Tracking.HeadPose;
			}
		}
	}
	else
	{
		...
		GlobeProgramTexture = GlTexture( GetTextureSwapChainHandle( chain, 0 ),
		...
		layer.DstBlend = VRAPI_FRAME_LAYER_BLEND_ZERO;
		layer.Flags &= ~VRAPI_FRAME_LAYER_FLAG_WRITE_ALPHA;
		//layer.Flags = VRAPI_FRAME_LAYER_FLAG_CHROMATIC_ABERRATION_CORRECTION;	// enable chromatic aberration correction on the pano
		for ( int eye = 0; eye < 2; eye++ )
		{
			layer.Textures[eye].ColorTextureSwapChain = FrameInput.ColorTextureSwapChain[eye];
			layer.Textures[eye].DepthTextureSwapChain = FrameInput.DepthTextureSwapChain[eye];
