...
Matrix4f SceneManager::BoundsScreenMatrix( const Bounds3f & bounds, const float movieAspect ) const
{
	...
	const float rotateAngle = ( size.x > size.z ) ? 0.0f : Mathf::Pi * 0.5f;
...
void SceneManager::Frame( const ovrFrameInput & vrFrame )
{
	...
	Scene.Frame( vrFrameWithoutMove, Cinema.app->GetHeadModelParms(), SceneInfo.UseFreeScreen ? 0 : -1 );
	...
	if ( SceneInfo.UseDynamicProgram )
	{
		...
		const unsigned int lightingTexId =
			GetTextureSwapChainHandle( MipMappedMovieTextureSwapChain, CurrentMipMappedMovieTexture );
		...
		// update the uniform values on the scene surfacedefs
		{
			if ( Scene.GetWorldModel().Definition != NULL )
			{
				ModelDef & def = *const_cast< ModelDef * >( &Scene.GetWorldModel().Definition->Def );
				for ( int i = 0; i < def.surfaces.GetSizeI(); i++ )
				{
	...
	ovrFrameParms & frameParms = Cinema.GetFrameParms();
	frameParms = vrapi_DefaultFrameParms( Cinema.app->GetJava(), VRAPI_FRAME_INIT_DEFAULT, vrapi_GetTimeInSeconds(), NULL );
	...
	if ( !drawScreen )
	{
		frameParms.LayerCount = 1;
		{
			...
			{
				layer.Textures[eye].ColorTextureSwapChain = Cinema.GetFrame().ColorTextureSwapChain[eye];
				layer.Textures[eye].DepthTextureSwapChain = Cinema.GetFrame().DepthTextureSwapChain[eye];
				layer.Textures[eye].TextureSwapChainIndex = Cinema.GetFrame().TextureSwapChainIndex;
				layer.Textures[eye].TexCoordsFromTanAngles = Cinema.GetFrame().TexCoordsFromTanAngles;
				layer.Textures[eye].HeadPose = Cinema.GetFrame().Tracking.HeadPose;
			}
		}
		return;
	}
	...
	if ( !GetUseOverlay() || SceneInfo.LobbyScreen || ( SceneInfo.UseScreenGeometry && ( SceneScreenSurface != NULL ) ) )
	{
		...
		ScreenTexture[0] = GlTexture( GetTextureSwapChainHandle( MipMappedMovieTextureSwapChain, CurrentMipMappedMovieTexture ), GL_TEXTURE_2D, 0, 0 );
	...
	else
	{
		...
		frameParms.LayerCount = 2;
		{
			...
			layer.SrcBlend = VRAPI_FRAME_LAYER_BLEND_ONE;
			layer.DstBlend = VRAPI_FRAME_LAYER_BLEND_ZERO;
			layer.Flags |= VRAPI_FRAME_LAYER_FLAG_WRITE_ALPHA;	// no chromatic aberration correction on the theater
			for ( int eye = 0; eye < 2; eye++ )
			{
				layer.Textures[eye].ColorTextureSwapChain = Cinema.GetFrame().ColorTextureSwapChain[eye];
				layer.Textures[eye].DepthTextureSwapChain = Cinema.GetFrame().DepthTextureSwapChain[eye];
		...
		{
			ovrFrameLayer & layer = frameParms.Layers[1];
			...
			for ( int eye = 0; eye < 2; eye++ )
			{
				...
				layer.Textures[eye].TexCoordsFromTanAngles = texMatrix[eye] * ovrMatrix4f_TanAngleMatrixFromUnitSquare( &modelViewMatrix );
				layer.Textures[eye].TextureRect = texRect[eye];
				layer.Textures[eye].HeadPose = Cinema.GetFrame().Tracking.HeadPose;
			}
		}
	}
