...
Created     :   June, 2015
Notes       :   StreamProcessor is used for decoding Capture streams on the remote
                client or command-line analysis tools.
...
namespace Capture
{
	...
	StreamProcessor::StreamProcessor(void)
	{
		...
		m_numPacketTypes                = 0;
		m_packetDescriptors             = nullptr;
		m_packetProcessors              = nullptr;
		m_streamThreadID                = 0;
	...
	bool StreamProcessor::ProcessData(const void *buffer, size_t bufferSize)
	{
		...
		if(!m_hasReadConnectionHeader && std::distance(curr, end) > (DataVector::difference_type)sizeof(ConnectionHeaderPacket))
		{
			...
			MemoryCopy(&connectionHeader, &*curr, sizeof(connectionHeader));
		...
		if(!m_hasReadPacketDescriptorHeader && std::distance(curr, end) > (DataVector::difference_type)sizeof(PacketDescriptorHeaderPacket))
		{
			...
			MemoryCopy(&packetDescHeader, &*curr, sizeof(packetDescHeader));
		...
		if(!m_hasReadPacketDescriptors && std::distance(curr, end) > (DataVector::difference_type)(sizeof(PacketDescriptorPacket)*m_numPacketTypes))
		{
			...
			{
				MemoryCopy(&m_packetDescriptors[i], &*curr, sizeof(PacketDescriptorPacket));
		...
		while(curr < end)
		{
			...
			if(curr == streamEnd && sizeof(StreamHeaderPacket) <= (size_t)(end - curr))
			{
				...
				MemoryCopy(&streamHeader, &*curr, sizeof(streamHeader));
	...
	return_failure:
		// Just mark the StreamProcessor as failed to prevent future ProcessData() calls from doing anything.
		m_hasFailure = false;
		return false;
	}
	...
	template<>
	void StreamProcessor::DispatchPacket<ThreadNamePacket_<1> >(const ThreadNamePacket_<1> &packet, const void *payloadData, size_t payloadSize)
	{
		onThreadName(m_streamThreadID, (const char*)payloadData, payloadSize);
	}

	template<>
	void StreamProcessor::DispatchPacket<ThreadNamePacket_<2> >(const ThreadNamePacket_<2> &packet, const void *payloadData, size_t payloadSize)
	{
		onThreadName(packet.threadID, (const char*)payloadData, payloadSize);
	}
	...
	template<>
	void StreamProcessor::DispatchPacket<MemoryAllocPacket>(const MemoryAllocPacket &packet)
	{
		onMemoryAlloc(m_streamThreadID, TimestampToSeconds(packet.timestamp), packet.size, packet.ptr);
	}

	template<>
	void StreamProcessor::DispatchPacket<MemoryReallocPacket>(const MemoryReallocPacket &packet)
	{
		onMemoryRealloc(m_streamThreadID, TimestampToSeconds(packet.timestamp), packet.size, packet.oldPtr, packet.newPtr);
	}

	template<>
	void StreamProcessor::DispatchPacket<MemoryFreePacket>(const MemoryFreePacket &packet)
	{
		onMemoryFree(m_streamThreadID, TimestampToSeconds(packet.timestamp), packet.ptr);
	}

	template<>
	void StreamProcessor::DispatchPacket<HeadTransformPacket>(const HeadTransformPacket &packet)
	{
		onHeadTransform(TimestampToSeconds(packet.timestamp), packet.orientation, packet.position);
	}

	template<>
	void StreamProcessor::DispatchPacket<CPUContextSwitch>(const CPUContextSwitch &packet)
	{
		const double t0 = TimestampToSeconds(packet.timestampEnter);
		const double t1 = TimestampToSeconds(packet.timestampLeave);
		onCPUContextSwitch(t0, t1, packet.threadID, packet.cpuID);
	}

	template<>
	void StreamProcessor::DispatchPacket<ButtonParamPacket>(const ButtonParamPacket &packet)
	{
		onNewButton(packet.labelID);
	}
	...
	size_t StreamProcessor::ProcessPacket(StreamProcessor &p, DataVector::iterator curr, DataVector::iterator bufferEnd)
	{
		...
		// Load the packet...
		MemoryCopy(&packet, &*curr, sizeof(packet));
	...
	size_t StreamProcessor::ProcessPacketWithPayload(StreamProcessor &p, DataVector::iterator curr, DataVector::iterator bufferEnd)
	{
		...
		MemoryCopy(&payloadSize, &*curr, sizeof(payloadSize));
		...
		if(payloadSize > 0)
		{
			...
			else
			{
				...
				MemoryCopy(payloadAligned, payloadUnaligned, payloadSize);
			}
		}
	...
	size_t StreamProcessor::LoadAndProcessNextPacket(DataVector::iterator curr, DataVector::iterator bufferEnd)
	{
		...
		// Peek at the header...
		MemoryCopy(&packetHeader, &*curr, sizeof(packetHeader));
	...
	StreamProcessor::ProcessPacketFunc StreamProcessor::GetProcessPacketFunction(UInt32 packetID, UInt32 version)
	{
		...
		{
			PacketFuncDesc::Create<ThreadNamePacket_<1> >(),
			PacketFuncDesc::Create<ThreadNamePacket_<2> >(),
			...
			PacketFuncDesc::Create<MemoryAllocPacket>(),
			PacketFuncDesc::Create<MemoryReallocPacket>(),
			PacketFuncDesc::Create<MemoryFreePacket>(),
			PacketFuncDesc::Create<HeadTransformPacket>(),
			PacketFuncDesc::Create<CPUContextSwitch>(),
			PacketFuncDesc::Create<ButtonParamPacket>(),
		};
