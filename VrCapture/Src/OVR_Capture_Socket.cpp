...
namespace Capture
{
	...
	{
		MemorySet(&m_addr, 0, sizeof(m_addr));
	}
	...
	#if defined(OVR_CAPTURE_POSIX)
		const int sret = ::select((m_socket>m_recvPipe?m_socket:m_recvPipe)+1, &readfd, &writefd, nullptr, (useTimeout ? &timeout : nullptr));
	#elif defined(OVR_CAPTURE_WINDOWS)
		const int sret = ::select(((int)m_socket)+1, &readfd, &writefd, nullptr, (useTimeout ? &timeout : nullptr));
