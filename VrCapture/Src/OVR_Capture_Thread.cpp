...
Notes       :   TODO: Switch to C++11 Primitives now thats what we build against.
Author      :   James Dolan
...
namespace Capture
{
	...
	#elif defined(OVR_CAPTURE_LINUX)
	...
	void GetThreadName(UInt32 threadID, char *buffer, size_t bufferSize)
	{
		OVR_CAPTURE_ASSERT(bufferSize >= 16);
	#if defined(OVR_CAPTURE_ANDROID) || defined(OVR_CAPTURE_LINUX)
		char commpath[32] = {};
		FormatString(commpath, sizeof(commpath), "/proc/self/task/%u/comm", threadID);
		ReadFileLine(commpath, buffer, bufferSize);
		...
		i = threadID;
		pthread_getname_np(pthread_from_mach_thread_np(t), buffer, bufferSize);
	#else
		FormatString(buffer, bufferSize, "Thread-%u", threadID);
	#endif
	}
